...
const ExpressionExercises: React.FC = () => {
    ...
    const [showExpressionResults, setShowExpressionResults] = useState<boolean[]>(Array(expressionExercisesVariants.length).fill(false));

    const exercises = expressionExercisesVariants[currentVariant];
    ...
    const isExpressionCorrect = (expr: string) => {
        ...
        const correctAns = (exercises as any)[expr];
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6 flex items-center justify-center">
            <div className="max-w-6xl w-full">
                ...
                <div className="flex gap-2 mb-6 justify-center">
                    {expressionExercisesVariants.map((_, index) => (
                        <button
                            key={index}
                                            ...
                                            <td className="border border-gray-300 p-3">
                                                ...
                                                {showExpressionResults[currentVariant] && hasAnswer && !isCorrect && (
                                                    <span className="text-xs text-red-600 mt-1 block">
                                                        Corect: <strong>{(exercises as any)[expr].type}</strong>
                                                    </span>
                                                )}
                                            </td>
                                            <td className="border border-gray-300 p-3">
                                                ...
                                                {showExpressionResults[currentVariant] && hasAnswer && !isCorrect && (
                                                    <span className="text-xs text-red-600 mt-1 block">
                                                        Corect: <strong>{(exercises as any)[expr].value.toString()}</strong>
                                                    </span>
                                                )}
                                            </td>
